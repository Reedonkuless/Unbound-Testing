...
import speciesBaseStats from './speciesBaseStats.json.js';
import speciesIdentityMeta from './speciesIdentityMeta.json.js';
import speciesGrowthRates from './speciesGrowthRates.json.js';
import speciesAbilitiesMeta from './speciesAbilitiesMeta.json.js';
import abilitiesCatalog from './abilitiesCatalog.json.js';
...
function resolveCurrentAbility(currentIndex, ability1Id, ability1Name, ability2Id, ability2Name, abilityHiddenId, abilityHiddenName, speciesId = null) {
    if (Number(speciesId) === 1302 && Number(currentIndex) === 0 && Number(ability1Id || 0) === 0 && Number(ability2Id || 0) === 0 && Number(abilityHiddenId || 0) === 0) {
        return { effective_ability_id: null, effective_ability_name: 'Eclipse Heart', ability_name_current: 'Eclipse Heart', ability_label_current: 'Eclipse Heart (Virtual)' };
    }
    if (currentIndex === 0) {
...
// Low-level utility for deterministic fixtures and format-preserving tools.
// The editor UI intentionally does not expose arbitrary experience editing.
export function updatePartyExperience(buffer, monIndex, payload) {
    mutatePartyMon(buffer, monIndex, (rawMon) => {
        const experience = Math.max(0, Math.min(0xFFFFFFFF, Number(payload?.experience || 0)));
        setExp(rawMon, experience);
        if (payload?.visual_level !== undefined) {
            const visualLevel = Math.max(1, Math.min(100, Number(payload.visual_level)));
            wu8(rawMon, OFF_LEVEL_VISUAL, visualLevel);
        }
        recalculatePartyStats(rawMon, true);
    });
}
