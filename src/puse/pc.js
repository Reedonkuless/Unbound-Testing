...
function getMonBufferAndOffset(context, box, slot) {
    ...
    if (hasFallback && context.sourceBuffer && (off + MON_SIZE_PC > context.pcBuffer.length || !isValidMon(context.pcBuffer.slice(off, off + MON_SIZE_PC)))) {
        ...
        if (Number.isInteger(absOff) && absOff + MON_SIZE_PC <= context.sourceBuffer.length) {
            const pending = context.absoluteEdits?.get(absOff);
            if (pending) return { buffer: pending, offset: 0, kind: 'absolutePending', absoluteOffset: absOff };
...
export function editPcMonFull(context, payload) {
    const box = Number(payload.box);
    const slot = Number(payload.slot);
    const { buffer, offset, kind, absoluteOffset } = getMonBufferAndOffset(context, box, slot);
    ...
    if (kind === 'absolute' || kind === 'absolutePending') {
        const absOff = absoluteOffset ?? offset;
        context.absoluteEdits.set(absOff, raw);
        const sectorOff = Math.floor(absOff / SECTION_SIZE) * SECTION_SIZE;
        if (shouldTrackAbsoluteSectorForChecksum(context.sourceBuffer, sectorOff)) {
            context.absoluteTouchedSectors.add(sectorOff);
